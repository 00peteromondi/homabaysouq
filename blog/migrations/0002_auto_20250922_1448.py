# blog/migrations/0002_default_blog_categories.py
from django.db import migrations

def add_default_blog_categories(apps, schema_editor):
...
def remove_default_blog_categories(apps, schema_editor):
    BlogCategory = apps.get_model('blog', 'BlogCategory')
    # Remove only the default categories we created
    default_slugs = [
        'technology', 'business-entrepreneurship', 'lifestyle', 'travel',
        'food-cooking', 'health-wellness', 'arts-culture', 'education',
        'personal-finance', 'news-current-events', 'diy-crafts', 'parenting',
        'sports', 'entertainment', 'science-nature', 'relationships',
        'home-garden', 'career-development', 'product-reviews', 'inspirational'
    ]
    BlogCategory.objects.filter(slug__in=default_slugs).delete()

class Migration(migrations.Migration):
    dependencies = [
        ('blog', '0001_initial'),  # Adjust this based on your actual previous migration
    ]

    operations = [
        migrations.RunPython(add_default_blog_categories, remove_default_blog_categories),
    ]